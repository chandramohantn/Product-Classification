...
def get_additional_vector(i_f, attr):
	data = []
	with open(i_f, 'r', encoding='utf-8') as f:
		for line in f:
			line = line[0:-1]
...
def get_breadcrumb_vector(i_f, attr):
	data = []
	with open(i_f, 'r', encoding='utf-8') as f:
		for line in f:
			line = line[0:-1]
...
def main():
	...
	i_f = 'filtered_additional.txt'
	additional_vect = get_additional_vector(i_f, additional_feat)
	i_f = 'filtered_breadcrumbs.txt'
	breadcrumb_vect = get_breadcrumb_vector(i_f, breadcrumb_feat)
	data = np.concatenate((additional_vect, breadcrumb_vect), axis=1)
